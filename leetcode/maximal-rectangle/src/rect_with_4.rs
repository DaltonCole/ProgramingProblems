...
impl Rect {
    ...
    pub fn area(&self) -> usize {
        (self.y2 + 1 - self.y1) * (self.x2 + 1 - self.x1)
    }

    pub fn overlapping(&self, x1: usize, x2: usize) -> AmountOverlap {
        // Perfect overlap
        if self.x1 == x1 && self.x2 == x2 {
            return AmountOverlap::Perfect(Rect::new(self.x1, self.y1, self.x2, self.y2 + 1));
        }
        // Inside - New row is inside the old row
        else if self.x1 <= x1 && x2 <= self.x2 {
            return AmountOverlap::Perfect(Rect::new(x1, self.y1, x2, self.y2 + 1));
        }
        // Shifted left
        else if x1 <= self.x1 && self.x1 <= x2 && x2 <= self.x2 {
            // Keep what we can of the old
            return AmountOverlap::Partial(Rect::new(self.x1, self.y1, x2, self.y2 + 1));
        }
        // Shifted right
        else if self.x1 <= x1 && x1 <= self.x2 && self.x2 <= x2 {
            // Keep what we can of the old
            return AmountOverlap::Partial(Rect::new(x1, self.y1, self.x2, self.y2 + 1));
        }
        // Outside - New row is wider and fully encompasses
        else if x1 <= self.x1 && self.x2 <= x2 {
            // Keep what we can of the old
            return AmountOverlap::Partial(Rect::new(self.x1, self.y1, self.x2, self.y2 + 1));
        }
    ...
    pub fn maximal_rectangle(matrix: Vec<Vec<char>>) -> i32 {
        ...
        for (y, row) in matrix.iter().enumerate() {
            ...
            for section in ones_section {
                ...
                if !perfect_overlap {
                    new_candidates.push(Rect::new(section.0, y, section.1, y));
                }
            }

            // Find all of the areas
            for candidate in candidates.iter() {
                max_area = std::cmp::max(max_area, candidate.area())
            }
