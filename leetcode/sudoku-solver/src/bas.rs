...
impl BoardState {
    ...
    pub fn next_num_possible_numbers(&self, row_num: usize, col_num: usize, prev_num: u8) -> usize {
        if prev_num == 0 {
            self.num_possible_numbers(row_num, col_num)
        } else {
            9 - u16::count_ones((self.possible_numbers(row_num, col_num) & ((1 << prev_num) - 1)))
                as usize
        }
    }
    ...
    pub fn get_next_possible_num(&self, row_num: usize, col_num: usize, prev_num: u8) -> u8 {
        (self.possible_numbers(row_num, col_num) & ((1 << prev_num) - 1)).trailing_ones() as u8 + 1
    }
    ...
    pub fn get_best_box_and_value(
        ...
        for row in prev_row..9 {
            for col in prev_col..9 {
                if self.board[row][col] == 0 {
                    //let num_possible = self.num_possible_numbers(row, col);
                    let num_possible = self.next_num_possible_numbers(row, col, prev_num);
                    if num_possible > 0 && num_possible < best_possibilities {
                        best_row = row;
                        best_col = col;
                        //best_num = self.get_next_possible_num(row, col, prev_num);
...
impl Solution {
    pub fn solve_sudoku(board: &mut Vec<Vec<char>>) {
        ...
        //let mut state_stack = Vec::from([((0, 0), 0)]);
        let mut last_row = 0;
        let mut last_col = 0;
        let mut last_num = 0;
        let mut a = 0;
        ...
        while !board_state.solved() {
            ...
            let ((mut best_row, mut best_col), mut num) =
                board_state.get_best_box_and_value(last_row, last_col, last_num);

            dbg!(state_stack.len());
            dbg!(board_state.get_best_box_and_value(last_row, last_col, last_num));
            ...
            while num == 0 {
                ((last_row, last_col), last_num) = state_stack.pop().unwrap();
                board_state.undo_move(last_row, last_col);
                dbg!(last_num);

                ((best_row, best_col), num) =
                    board_state.get_best_box_and_value(last_row, last_col, last_num);
                dbg!(board_state.get_best_box_and_value(last_row, last_col, last_num));
            }

            board_state.make_move(best_row, best_col, num);
            state_stack.push(((best_row, best_col), num));
        }
...
mod test {
    ...
    fn board_state_test() {
        ...
        // Best box and num
        dbg!(board_state.print());
        assert_eq!(((4, 4), 5), board_state.get_best_box_and_value(0, 0, 0));
